...
function Offer({ data, className }) {
  ...
  const description = () => {
    if (isDisabled) {
      return (
        <p className='offer__description offer__description_disabled'>
          Sorry, the discount is unavailable
        </p>
      );
    }
    ...
    return (
      <p className='offer__description'>
        Get an extra <span className='offer__descriptionSelected'>FREE</span>{' '}
        gutter cleaning!
      </p>
    );
  };
  ...
  return (
    <div className={classFull}>
      <div
        className='offer__block offer__block_usa'
        onClick={onClick}
      ...
      >
        <div className='offer__wrapper'>
          ...
          {isHover && isSelected ? (
            <p className='offer__subtitle'>Excellent decision!</p>
          ) : (
            <p className='offer__subtitle'>{data.subtitle}</p>
          )}
          <div className='offer__imgBlock'>
            {data.salePercentage ? (
              <div className='offer__label'>
                <span className='offer__labelTop'>{data.salePercentage}</span>
                <span className='offer__labelBottom'>%OFF</span>
              </div>
            ) : null}
            <picture>
              {data.img.srcWebp ? (
                <source srcSet={data.img.srcWebp} type='image/webp' />
              ) : null}
              <img
                className='offer__img'
                srcSet={data.img.srcJpg}
                alt={data.img.title}
                type='image/jpeg'
